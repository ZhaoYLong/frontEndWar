...
const attribute = /^\s*([^\s"'<>\/=]+)(?:\s*(=)\s*(?:"([^"]*)"+|'([^']*)'+|([^\s"'=<>`]+)))?/  // 表签属性
...
const endTag = new RegExp(`^<\\/${qnameCapture}[^>]*>`)  // 结束biaoq
const doctype = /^<!DOCTYPE [^>]+>/i  // <!DOCTYPE html>
// #7298: escape - to avoid being passed as HTML comment when inlined in page
const comment = /^<!\--/  // 注释的正则
const conditionalComment = /^<!\[/  // 条件注释符

// Special Elements (can contain anything)
export const isPlainTextElement = makeMap('script,style,textarea', true)
const reCache = {}

const decodingMap = {  // 解码Map
  '&lt;': '<',
  '&gt;': '>',
  '&quot;': '"',
  '&amp;': '&',
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      if (textEnd === 0) {
        ...
        if (endTagMatch) {
          ...
          parseEndTag(endTagMatch[1], curIndex, index)  // 调用parseEndTag()解析器解析关闭标签
          continue
        }
