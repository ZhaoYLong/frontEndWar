...
## Web Storage
> Web Storage的目的是解决通过客户端存储不需要频繁发送回服务器的数据时，使用cookie的问题，其提供了cookie之外的存储会话数据的途径和跨会话持久化存储大量数据的机制，其主要有两个对象：localStorage（永久存储机制）和sessionStorage（跨会话存储机制）。

### sessionStorage
- 特点：
  - 1.sessionStorage对象值存储会话数据，其生命周期会存储到浏览器关闭。（在该过程中刷新其数据不受影响）
  - 2.浏览器在实现存储写入时使用同步阻塞方式，数据会被立即提交到存储。
  - 3.独立打开同一个窗口同一个页面或一个Tab，sessionStorage也不一样。
  - 存储空间大小限制为每个源不超过5M。
...
```js
  ...
  // 删除
  sessionStorage.removeItem('ssessionName1')
...
```js
  ...
  localStorage.getItem('localName1')
  localStorage.localName2
...
## IndexedDB
![IndexedDB](https://mmbiz.qpic.cn/mmbiz_png/q4qrl2ddrUuoYjoSUmbpBgEvQLKK4WBeiaWceANBFh3LoYKPwWluHOHficnviajyAg5UvQVQrhTP6JZXhGSRBzIJA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

- 对于整个IndexedDB的描述：
  - 1.一个域名下可以包含多个数据库；
  - 2.一个数据库中包含多个对象仓库，类似于Mysql一个库中有多个表
  - 3.每个对象仓库中包含多条数据记录

### 主要特点
- IndexedDB是浏览器中存储结构化数据的一个方案，其设计几乎是完全异步的，主要特点：
  ...
  - 6.支持二进制存储不仅可以存储字符串，还可以存储二进制数据（ArrayBuffer和Blob）

### 数据库操作

1. 初始化数据库，传入创建的数据库名和版本，获取对应的数据库操作实例
2. 对象仓库操作
  - 数据是在对象仓库中存储的，创建数据库后则需要创建所需的数据仓库
3. 数据操作
  - 不管关系型数据库还是非关系型数据库，CURD肯定必不可少。
4. 遍历内容
5. 调用代码
